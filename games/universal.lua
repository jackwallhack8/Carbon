			...
			if callback then
				...
				Invisible:Clean(runService.PreSimulation:Connect(function(dt)
					if entitylib.isAlive and oldroot then
						...
						local totalHeight = humanoid.HipHeight + (root.Size.Y / 2)
						local cf = root.CFrame - Vector3.new(0, totalHeight + 4, 0)
	...
	ChatSpammer = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'carbonv4 on top')

...
local loadstring = function(...)
	...
	if err and vape then
		vape:CreateNotification('CarbonV4', 'Failed to load : '..err, 30, 'alert')
	end
	return res
end
do
	...
	if _hookDetected then
		game:GetService('Players').LocalPlayer:Kick('[CARBONV4] integrity check failed - what u trying to do???')
		error('[CARBONV4] loadstring hook detected - if this is false dm Sharpie837', 2)
	end
end
	...
	if not isfile(path) then
		...
		for attempt = 1, 3 do
			local suc, result = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/jackwallhack8/Carbon/' .. readfile('newvape/profiles/commit.txt') .. '/' .. select(1, path:gsub('newvape/', '')), true)
			end)
...
local function finishLoading()
	...
	if not vape.Load then
		warn('[CARBONV4] vape.Load is nil skipping load')
		return
	end
	vape:Load()
	...
	vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				loadstring(game:HttpGet('https://raw.githubusercontent.com/jackwallhack8/Carbon/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
			]]
			...
			if not _ok then warn('[CARBONV4] save failed before teleport: ' .. tostring(_err)) toclipboard(_err) end
		...
		if vape.Categories.Main.Options['GUI bind indicator'].Enabled then
			...
			task.spawn(function()
				...
				while tick() < deadline do
					if getgenv()._carbonTierReady then break end
					task.wait(0.5)
				end
				local tier = 0
				if getgenv().getCarbonTier then
					tier = getgenv().getCarbonTier(playersService.LocalPlayer) or 0
				end
				if tier == 0 then
					task.wait(3)
					if getgenv().getCarbonTier then
				...
				vape:CreateNotification('[CARBONV4] Finished Loading [Tier ' .. tostring(tier) .. ']', name .. (vape.VapeButton and 'Press the button in the top right to open GUI' or 'Press ' .. table.concat(vape.Keybind, ' + '):upper() .. ' to open GUI'), 5)
			end)
		end
	end
end
...
if not guiFunc then
	...
	error('[CARBONV4] syntax error in ' .. gui .. '.lua' .. '\n' .. errMsg .. context)
end
vape = guiFunc()
if not vape then
	error('[CARBONV4] GUI returned nil file may be corrupted try deleting newvape/guis/' .. gui .. '.lua and reinjecting.')
end
if not vape.Load then
	if delfile then pcall(function() delfile('newvape/guis/' .. gui .. '.lua') end) end
	error('[CARBONV4] gui file corrupted (missing load) reinject..')
end
if not vape.Init and not vape.Load then
	error('[CARBONV4] failed to initialize properly reinject to fix this bs')
end
shared.vape = vape
task.wait(0.1)

do
	getgenv()._carbonTierReady = true
	getgenv().getCarbonTier = function(player)
...
if not shared.VapeIndependent then
	...
	if isfile('newvape/games/' .. gameFileId .. '.lua') then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/jackwallhack8/Carbon/' .. readfile('newvape/profiles/commit.txt') .. '/games/' .. gameFileId .. '.lua', true)
			end)
